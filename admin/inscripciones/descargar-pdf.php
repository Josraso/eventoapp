...
$id = (int)($_GET['id'] ?? 0);
$stE = db()->prepare('SELECT e.*, i.numero_pedido, i.estado_pago FROM entradas e JOIN inscripciones i ON i.id=e.inscripcion_id WHERE e.id=?');
$stE->execute([$id]);
$entrada = $stE->fetch();
...
if (!$entrada['pdf_path'] || !file_exists(__DIR__ . '/../../' . $entrada['pdf_path'])) {
    ...
    TicketManager::guardarPDF($id, $pdfContent);
} else {
    $pdfContent = file_get_contents(__DIR__ . '/../../' . $entrada['pdf_path']);
}
